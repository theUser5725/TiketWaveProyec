...
Principios:
- Consistencia transaccional al reservar asientos mediante bloqueos explícitos (SELECT FOR UPDATE).
- Separación de responsabilidades: API, dominio puro y adaptadores/infra.
- Sistema extensible para notificaciones y almacenamiento en cache.
- Serialización de operaciones críticas mediante singleton process-wide.
...
- `ContextoReserva` — DbContext con mappings personalizados para PostgreSQL.
- `ReservaRepository` — implementación con transacciones y bloqueos explícitos (SELECT FOR UPDATE).
- `RedisCacheService` — servicio de cache distribuido usando Redis.
- `NotificationService` — cola en memoria + worker para notificaciones asíncronas.
- `ReservaSingletonService` — serializador de operaciones críticas usando `SemaphoreSlim`.
